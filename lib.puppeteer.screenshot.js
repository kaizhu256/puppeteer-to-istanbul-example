...
/**
  ...
  * @return {!Promise<!Browser>}
    */
Launcher.prototype.launch = async function () {
        var chromeProcess;
        ...
        const waitForChromeToClose = new Promise(function (fulfill, reject) {
                chromeProcess.once("exit", function () {
                        ...
                        if (temporaryUserDataDir) {
                                ...
                                ).then(fulfill).catch(console.error);
                        } else {
                                fulfill();
                        }
                });
        ...
        let connection = null;
        try {
                const browserWSEndpoint = await waitForWSEndpoint(chromeProcess, 30000, 674921);
                const transport = await WebSocketTransport.create(browserWSEndpoint);
                connection = new Connection(browserWSEndpoint, transport, 0);
                const browser = await Browser.create(connection, [], false, {width: 800, height: 600}, chromeProcess, gracefullyCloseChrome);
